...
from model import TableGan, RuleModel
...
flags.DEFINE_string("checkpoint_rule_model_dir", "", "directory of Rule Model ")

FLAGS = flags.FLAGS

TRAIN_RULE_MODEL = True
...
def main(_):
    ...
    if FLAGS.shadow_gan:
        checkpoint_tablegan_folder = FLAGS.checkpoint_par_dir + '/' + FLAGS.dataset + "/" + 'atk_' + FLAGS.test_id
        checkpoint_rulmodel_folder = "./checkpoint/"+FLAGS.dataset+"/ruleModel"
    else:
        checkpoint_tablegan_folder = f'{FLAGS.checkpoint_par_dir}/{FLAGS.dataset}/{FLAGS.test_id}/tableGan'
        checkpoint_rulmodel_folder = "./checkpoint/"+FLAGS.dataset+"/ruleModel"

    if not os.path.exists(checkpoint_tablegan_folder):
        os.makedirs(checkpoint_tablegan_folder)

    FLAGS.checkpoint_dir = checkpoint_tablegan_folder
    FLAGS.checkpoint_rule_model_dir = checkpoint_rulmodel_folder
    ...
    with tf.Session(config=run_config) as sess:
        # todo 训练一个 rule model 来评判 fake data 是否符合 rule
        ruleModel = RuleModel(sess=sess,
                              ...
                              crop=FLAGS.crop,
                              checkpoint_dir=FLAGS.checkpoint_rule_model_dir,
        ...
        if (not ruleModel.load(FLAGS.checkpoint_rule_model_dir)[0]) and FLAGS.batch_size == 500:
            ruleModel.train(FLAGS)
            print("rule Model 训练完成")
        # todo 计算一下 loss
        print("---------------------------------")
        # ruleModel.train(FLAGS)
        return
